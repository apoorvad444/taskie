...
import { addMemberByAdmin } from "@/actions/auth";
...
export default function AddMemberModal() {
  ...
  const onSubmit = async (data: FormData) => {
    setError(null);
    const result = await addMemberByAdmin(data);
    if (result.success) {
      reset();
      setOpen(false);
      router.refresh();
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-medium px-4 py-2 rounded-xl transition-all duration-200 shadow-lg shadow-indigo-600/20"
      >
          ...
          <div className="relative bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-md shadow-2xl mx-4">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                <select
                  {...register("role")}
                  className="w-full px-4 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition text-sm"
                >
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-800 disabled:cursor-not-allowed text-white text-sm font-medium py-2.5 rounded-xl transition-all duration-200"
                >
                  {isSubmitting ? (
